...
void MPU9250ReadAngles_Setup() {
  ...
  /* magcalMPU9250(magBias, magScale);   //note this function to avoid
   calibration Serial.print(magBias[0]); Serial.print("\t");
  ...
  // magcalMPU9250(magBias,magScale);    //note this function to avoid
  // calibration magBias[0] = -101.35; magBias[1] = 344.56; magBias[2] = 0.28;
  // // To skip calibration magScale[0] = 1.00; magScale[1] = 1.01; magScale[2]
  // = 0.99;    // See above ^

  magBias[0] = -79.63;
  magBias[1] = 308.48;
  magBias[2] = -10.4; // To skip calibration
  magScale[0] = 1.03;
  magScale[1] = 0.99;
  magScale[2] = 0.98; // See above ^
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void MPU9250Calculate(float deltat) {
  // On interrupt, check if data ready interrupt
  if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01) {
    readAccelData(accelCount); // Read the x/y/z adc values
    getAres();

    // Now we'll calculate the accaleration value into actual g's
    ax = (float)accelCount[0] * aRes; // - accelBias[0];  // get actual g value,
    ...
    //    milliGauss, should be automatically calculated magbias[1] = +120.;  //
    //    User environmental x-axis correction in milliGauss magbias[2] = +125.;
    ...
    // get actual magnetometer value, this depends on scale
    // being set
  ...
  pitch *= 180.0f / PI;
  yaw *= 180.0f / PI;
  yaw += 1.34; /* Declination at Potheri, Chennail ,India  Model Used:	IGRF12
                  Help 2016-04-09	1.34° W  changing by  0.06° E per year
                  (+ve for west )*/
  roll *= 180.0f / PI;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnYaw() { return yaw + 180.0f; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnPitch() { return pitch; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnRoll() { return roll; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnAccX() { return ax; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnAccY() { return ay; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnAccZ() { return az; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnGyrX() { return gx; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnGyrY() { return gy; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnGyrZ() { return gz; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnMagX() { return mx; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnMagY() { return my; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
float MPU9250_ReturnMagZ() { return mz; }
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void magcalMPU9250(float *dest1, float *dest2) {
  uint16_t ii = 0, sample_count = 0;
  int32_t mag_bias[3] = {0, 0, 0}, mag_scale[3] = {0, 0, 0};
  int16_t mag_max[3] = {0x8000, 0x8000, 0x8000},
          mag_min[3] = {0x7FFF, 0x7FFF, 0x7FFF}, mag_temp[3] = {0, 0, 0};
  // Blink(Cal_LED_Pin);
...
void readGyroData(int16_t *destination) {
  ...
  // Read the six raw data registers sequentially into
  // data array
...
void readMagData(int16_t *destination) {
  ...
  if (readByte(AK8963_ADDRESS, AK8963_ST1) & 0x01) {
    // Read the six raw data and ST2 registers
    // sequentially into data array
    readBytes(AK8963_ADDRESS, AK8963_XOUT_L, 7, &rawData[0]);
    uint8_t c = rawData[6]; // End data read by reading ST2 register
    // Check if magnetic sensor overflow set, if not then
    // report data
    if (!(c & 0x08)) {
      // Turn the MSB and LSB into a signed 16-bit value
...
int16_t readTempData() {
  ...
  // Read the two raw data registers sequentially into
  // data array
...
void initMPU9250() {
  ...
  // Auto select clock source to be PLL gyroscope reference if
  // ready else
  ...
  writeByte(
      ...
      MPU9250_ADDRESS, SMPLRT_DIV, 0x04);
  ...
  // writeRegister(GYRO_CONFIG, c | 0x00); // Set Fchoice for the gyro to 11 by
  // writing its inverse to bits 1:0 of GYRO_CONFIG
...
void calibrateMPU9250(float *dest1, float *dest2) {
  ...
  // Set gyro full-scale to 250 degrees per second, maximum
  // sensitivity
  ...
  // Enable gyro and accelerometer sensors for FIFO  (max size
  ...
  for (ii = 0; ii < packet_count; ii++) {
    ...
    // Sum individual signed 16-bit biases to get
  ...
  // Divide by 4 to get 32.9 LSB per deg/s to conform to
  // expected bias input format
  data[0] = (-gyro_bias[0] / 4 >> 8) & 0xFF;
  // Biases are additive, so change sign
  ...
  // Define mask for temperature compensation bit 0 of
  ...
  for (ii = 0; ii < 3; ii++) {
    if ((accel_bias_reg[ii] & mask))
      // If temperature compensation bit is set, record
  ...
  // preserve temperature compensation bit when
...
void MPU9250SelfTest(float *destination) {
  ...
  // get average current values of gyro and accalerometer
  ...
  // Enable self test on all three axes and set gyro range to
  // +/- 250 degrees/s
  ...
  for (int ii = 0; ii < 200;
       ii++) { // get average self-test values of gyro and accalerometer
  ...
  factoryTrim[0] =
      (float)(2620 / 1 << FS) * (pow(1.01, ((float)selfTest[0] - 1.0)));
  // FT[Ya] factory trim calculation
  factoryTrim[1] =
      (float)(2620 / 1 << FS) * (pow(1.01, ((float)selfTest[1] - 1.0)));
  // FT[Za] factory trim calculation
  factoryTrim[2] =
      (float)(2620 / 1 << FS) * (pow(1.01, ((float)selfTest[2] - 1.0)));
  // FT[Xg] factory trim calculation
  factoryTrim[3] =
      (float)(2620 / 1 << FS) * (pow(1.01, ((float)selfTest[3] - 1.0)));
  // FT[Yg] factory trim calculation
  factoryTrim[4] =
      (float)(2620 / 1 << FS) * (pow(1.01, ((float)selfTest[4] - 1.0)));
  // FT[Zg] factory trim calculation
  factoryTrim[5] =
      (float)(2620 / 1 << FS) * (pow(1.01, ((float)selfTest[5] - 1.0)));
