void Mission_5() {
  ...
  int Dist_Down = Dist_DownConst; // distance down the ramp [mm] || NEED TO
                                  // MEASURE MAZE TO CHANGE VALUE!
  int Dist_Wall = Dist_WallConst; // distance from ramp end to the wall [mm] ||
                                  // NEED TO MEASURE MAZE TO CHANGE VALUE!
  int approach = approachConst; // distance to coarse approach the pole [mm] ||
                                // NEED TO MEASUER MAZE TO CHANGE VALUE!
  ...
  while (Mission_Number == 5) {
    ...
    while (Dist_Down >
           0) { // || (Pitch > Pitch0 + 3)) { //while still on ramp & no Pitch
      noInterrupts();
    ...
    /*
      ...
      to the Right (pole side) delay(5000); noInterrupts(); dist_cntr = 0;
      interrupts();
      //************************************
      ...
      //ANGLE IN RAD*RADIUS = DISTANCE CROSSED Serial.print("Approach Dist:
      ...
      Turn_Left_In_Angle(90); // Rotate the robot in-place in 90 degrees to the
    ...
    */
    ...
    //************************************
    Turn_Right_In_Angle(
        90); // Rotate the robot in-place in 90 degrees to the right
    ...
    while (!Found2) {
      for (int i = 45; i < 135;
           i++) { // i is the servo angle, for loop scans with ToF
        Servo_Motor.write(i);
        Serial.println(TOF_Sensor.readRangeContinuousMillimeters());
    ...
    //************************************
    Move_Forwards(nominal_speed); // Go towards the pole
    while ((TOF_Sensor.readRangeContinuousMillimeters() <= dist + 50) &&
           (TOF_Sensor.readRangeContinuousMillimeters() >= 100)) {
      Serial.print(TOF_Sensor.readRangeContinuousMillimeters());
      Serial.println("\t CHARGE!!");
      // final_dist = 100 - dist_cntr * ENCODER_TO_ANGLE * WHEEL_RADIUS; //ANGLE
      // IN RAD*RADIUS = DISTANCE CROSSED
    }
    Stop();
    Serial.println("done :)");
...
/*
  ...
  XX 3.Rotate_ToF(angle) - global function that rotates the servo with ToF to
  the absolute degree in the parenthesis
